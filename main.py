...
def chat_with_kimi():
    ...
    data = request.get_json()
    prompt = data.get('prompt')
    model = data.get('model', config['models']['kimi']['default_model'])
    temperature = data.get('temperature', 0.7)
    max_tokens = data.get('maxTokens', 1024)
    ...
    try:
        # 初始化 Kimi 客户端
        client = OpenAI(
            api_key=config['models']['kimi']['api_key'],
            base_url=config['models']['kimi']['base_url']
        )
        ...
        record_history(
            ...
            duration=duration,
            file_name=None,
            description=f'Kimi AI分析 - 模型: {model}'
        )
        ...
        record_history(
            ...
            duration=duration,
            file_name=None,
            description=f'Kimi AI分析失败 - 模型: {model}'
        )
        
        return jsonify({
            'code': 1002,
            'message': f'调用 Kimi API 失败: {str(e)}'
        }), 500

# 调用 ModelScope 魔搭模型
@app.route('/modelscope', methods=['POST'])
def chat_with_modelscope():
    start_time = datetime.now()
    data = request.get_json()
    prompt = data.get('prompt')
    model = data.get('model', config['models']['modelscope']['default_model'])
    temperature = data.get('temperature', 0.7)
    max_tokens = data.get('maxTokens', 1024)
    ...
    try:
        # 初始化 ModelScope 客户端
        client = OpenAI(
            api_key=config['models']['modelscope']['api_key'],
            base_url=config['models']['modelscope']['base_url']
        )
        ...
        record_history(
            ...
            duration=duration,
            file_name=None,
            description=f'ModelScope AI分析 - 模型: {model}'
        )
        ...
        record_history(
            ...
            duration=duration,
            file_name=None,
            description=f'ModelScope AI分析失败 - 模型: {model}'
        )
        
        return jsonify({
            'code': 1002,
            'message': f'调用 ModelScope API 失败: {str(e)}'
        }), 500
